...
# scaling
samples_$GD <-scale(samples_$GD.mg.kg.min, center = TRUE, scale = TRUE)
samples_$IA <-scale(samples_$INS..mIU.l..Arter., center = TRUE, scale = TRUE)
samples_$IV <-scale(samples_$Insulin..mIU.l..Venous, center = TRUE, scale = TRUE)
samples_$TAG <-scale(samples_$TAG..mmol.l..Arter., center = TRUE, scale = TRUE)
samples_$FFA <-scale(samples_$FFA..mmol.l..Arter., center = TRUE, scale = TRUE)
samples_$GA <-scale(samples_$Glucose..mmol.l..Arter., center = TRUE, scale = TRUE)
...
dds <- DESeq(dds)
# dds <- DESeq(dds, test="LRT", reduced = ~ patient + tmp)

resultsNames(dds)

pheno <- "GD"
tmpPost <- paste(pheno, "tmpPost", sep=".")
tmpPre <- paste(pheno, "tmpPre", sep=".")

# res <- results(dds, contrast=c("tmp","Post","Pre"), alpha=0.05)
res <- results(dds, contrast=list(tmpPost, tmpPre), alpha=0.05)
res <- results(dds, name=tmpPost, alpha=0.05)
res <- results(dds, name=tmpPre, alpha=0.05)
...
data <- plotCounts(dds, gene=gene, intgroup=c("tmp", "patient"), returnData=TRUE, normalized=TRUE)
...
# to csv
# write.csv(res, "deseq2_results_LRT.csv", row.names = TRUE)
write.csv(res, "deseq2_results_GD.csv", row.names = TRUE)
