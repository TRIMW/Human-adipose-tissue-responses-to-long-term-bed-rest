...
results_fps <- list.files(
  path = "/Users/willtrim/Documents/projs/bedrest/outputs/cors2/", 
  pattern = ".*.csv$",
...
pheno_list <- c("pre_clamp_change_cho_ox", "pre_clamp_change_fat_ox", "post_clamp_change_cho_ox", "post_clamp_change_fat_ox")
...
setwd("/Users/willtrim/Documents/projs/bedrest/outputs/cors2/fgsea_res")
library(data.table)
...
for (pheno_name in all_phenotypes) {
  ...
  for (db_name in names(all_pathways)) {
    ...
    fwrite(
      fgseaRes,
      paste(pre_pheno, ".", db_name, ".csv", sep=""),
      row.names = FALSE
    )
    ...
    fwrite(
      fgseaRes,
      paste(post_pheno, ".", db_name, ".csv", sep=""),
      row.names = FALSE
    )
  }
}
